...
contract NFTContract is ERC721, ERC721Enumerable, ERC721URIStorage {
    ...
    function createSVG(uint256 tokenId) public pure returns (string memory) {
        bytes memory svg = abi.encodePacked(
            ...
            '<text x="50%" y="15%" class="base2" dominant-baseline="middle" text-anchor="middle">',
            "BICONOMY",
            "</text>"
    ...
    function getNFTURI(uint256 tokenId) public pure returns (string memory) {
        bytes memory dataURI = abi.encodePacked(
            "{",
            '"name": "B-NFT #',
            tokenId.toString(),
            '",',
            '"description": "Jefferson minting a gasless NFT powered by Biconomy SDK",',
            '"image": "',
            createSVG(tokenId),
            '",',
            '"linktree_url": "https://linktr.ee/biconomydevrel',
            "}"
        );
    ...
    function createNFT() public {
        ...
        _setTokenURI(tokenId, getNFTURI(tokenId));
    }

    function transferMyNFT(address recipient, uint256 tokenId) public {
